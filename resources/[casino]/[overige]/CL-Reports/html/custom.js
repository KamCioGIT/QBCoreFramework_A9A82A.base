  ...
  getReportStatus(playerID, function(status) {
    if (status) {
      $.post(`https://${GetParentResourceName()}/SendNotify`, JSON.stringify({
        message: 'U heeft al een actief rapport. Wacht tot uw melding is afgehandeld of totdat de time-out is verstreken.',
        type: 'error',
      }));
    } else {
      const dropdown = document.querySelector('.dropdown-input');
...
toggleBtn.addEventListener('click', () => {
  ...
  if (toggleBtn.classList.contains('active')) {
    $.post(`https://${GetParentResourceName()}/SendNotify`, JSON.stringify({
      message: 'Rapportmeldingen ingeschakeld. U wordt op de hoogte gesteld wanneer er nieuwe rapporten worden ingediend.',
      type: 'success',
    }));
    ...
    $.post(`https://${GetParentResourceName()}/SendNotify`, JSON.stringify({
      message: 'Rapportmeldingen uitgeschakeld. U wordt niet op de hoogte gehouden van nieuwe meldingen.',
      type: 'primary',
    }));
    $.post(`https://${GetParentResourceName()}/SetReceiveReports`, JSON.stringify({
