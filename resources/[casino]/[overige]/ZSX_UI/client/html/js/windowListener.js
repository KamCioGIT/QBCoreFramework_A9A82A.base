window.addEventListener("message", async e=> {
    ...
    } else if (NUI.type == 'CANCEL_PROGRESSBAR') {
        ZSX.Functions.Game.Progress.Cancel()
    } else if (NUI.type == 'START_UI_MULTICHARACTER') {
        ZSX.Variables.CfgToJs.MulticharacterInitialized = true
        if (!ZSX.Variables.CfgToJs.ConfigurationMenu) {
            ZSX.Functions.Game.OpenAfterConfigWithoutInterface(false)
            await sleep(3000)
            ZSX.Variables.Active.Game = true 
            $.post(`https://${GetParentResourceName()}/UpdateState`, JSON.stringify({element: "Game", state: true}))
            $(".logo-screen").fadeOut(1000)
            ...
            if (ZSX.Functions.Boot.Storage()) {
                ...
                $.post(`https://${GetParentResourceName()}/focus`, JSON.stringify({state: true}))
                ZSX.Functions.Welcome.Initialize()
            }
        }
    }
