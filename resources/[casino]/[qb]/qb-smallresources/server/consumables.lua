...
QBCore.Functions.CreateUseableItem('weed_skunk_cbd_joint', function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player.Functions.RemoveItem(item.name, 1, item.slot) then return end
    TriggerClientEvent('consumables:client:Useweed_skunk_cbd_joint', source)
end)

QBCore.Functions.CreateUseableItem('weed_white_widow_cbd_joint', function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player.Functions.RemoveItem(item.name, 1, item.slot) then return end
    TriggerClientEvent('consumables:client:Useweed_white_widow_cbd_joint', source)
end)

QBCore.Functions.CreateUseableItem('weed_og_kush_cbd_joint', function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player.Functions.RemoveItem(item.name, 1, item.slot) then return end
    TriggerClientEvent('consumables:client:Useweed_og_kush_cbd_joint', source)
end)

QBCore.Functions.CreateUseableItem('weed_ak47_cbd_joint', function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player.Functions.RemoveItem(item.name, 1, item.slot) then return end
    TriggerClientEvent('consumables:client:Useweed_ak47_cbd_joint', source)
end)
