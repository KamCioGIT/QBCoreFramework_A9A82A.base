...
CreateThread(function()
	while true do
		...
		if dist <= 25.0 then
			...
			if dist <= 2.0 then
				DrawText3D(Config.MissionMarker.x, Config.MissionMarker.y, Config.MissionMarker.z, "~b~[E]~w~ Om de missie te aanvaarden")
	...
	if PlayerJob ~= nil and PlayerJob.name == 'police' then
		...
		if PoliceDist <= 4.5 then
			...
			if SilenceAlarm == 0 then
				hintToDisplay('Druk op ~INPUT_DETONATE~ om het alarm uit te zetten')
				SilenceAlarm = 1
			end
...
RegisterNetEvent('qb-armoredtruckheist:client:robberyCall', function(streetLabel, coords)
	if PlayerJob.name == "police" then
		...
		TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
			timeOut = 10000,
			alertTitle = "Poging tot overval op een gepantserde vrachtwagen",
			coords = {
				x = coords.x,
				y = coords.y,
				z = coords.z,
...
function MissionNotification()
	...
	TriggerServerEvent('qb-phone:server:sendNewMail', {
		sender = "De Baas",
		subject = "Nieuw doel",
		message = "Dus je bent geïnteresseerd om wat geld te verdienen? Goed... koop een wapen en zorg dat het gebeurt... Ik stuur je nu de locatie.",
	})
end
---
--
		...
		while MissionStart == 1 do
			...
			if dist <= 55.0  then
				...
				if warning == 0 then
					warning = 1
					stopAndBeAngry()
					QBCore.Functions.Notify("Schakel de bewakers uit voordat je de bom plaatst.", "error")
					AlertPolice()
				end
				...
				if dist <= 7 and BlownUp == 0 and PlayerJob.name ~= 'police' then
					if BlowBackdoor == 0 then
						hintToDisplay('Druk op [G] om de achterdeur op te blazen en het geld te pakken')
	...
	if IsVehicleStopped(transport) then
		if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and GuardsDead == 1 then
			if not IsEntityInWater(PlayerPedId()) then
				...
				BlownUp = 1
				lootable = 1
				QBCore.Functions.Notify('U kunt beginnen met het verzamelen van contant geld.', "success")
				RemoveBlip(TruckBlip)
			else
				QBCore.Functions.Notify('Ga uit het water', "error")
			end
		else
			QBCore.Functions.Notify('Het voertuig moet leeg zijn om de lading te kunnen plaatsen', "error")
		end
	else
		QBCore.Functions.Notify('Je kunt een rijdend voertuig niet beroven.', "error")
	end
end
...
CreateThread(function()
    while true do
		...
		if lootable == 1 then
			...
			if dist <= 4.5 then
				sleep = 0
				hintToDisplay('Druk op [E] om het geld te nemen')
...
function TakingMoney()
	...
	FreezeEntityPosition(PlayerPedId(), true)
	QBCore.Functions.Notify('Je stopt contant geld in een zak', "success")
	...
	while GetGameTimer() - _time < 20000 do
		...
		hintToDisplay('Houd [G] vast om te ontsnappen')
		Wait(0)
	end
