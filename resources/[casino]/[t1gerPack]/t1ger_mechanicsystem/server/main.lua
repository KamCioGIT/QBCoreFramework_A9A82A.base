...
AddEventHandler('mechanicsystem:server:playerLoaded', function()
    ...
    TriggerClientEvent('mechanicsystem:client:setupShopMarkers', src, mech_shops)
...
AddEventHandler('mechanicsystem:server:createMarker', function(input, args)
    ...
    TriggerClientEvent('mechanicsystem:client:updateShops', -1, mech_shops)
    TriggerClientEvent('mechanicsystem:client:addShopMarker', -1, args.data.id, args.type, marker)
...
AddEventHandler('mechanicsystem:server:deleteMarker', function(args)
    ...
    mech_shops[args.data.id].markers = args.data.markers
    TriggerClientEvent('mechanicsystem:client:updateShops', -1, mech_shops)
    TriggerClientEvent('mechanicsystem:client:removeShopMarker', -1, args.data.id, args.type)
    Core.Notification(src, {
        title = '',
...
AddEventHandler('mechanicsystem:server:createLift', function(data, lift)
    local src = Core.Player.GetSource(source)
...
AddEventHandler('mechanicsystem:server:deleteShop', function(data)
    ...
    MySQL.Async.execute('DELETE FROM '..database_table..' WHERE id = @id', {
        ...
        TriggerClientEvent('mechanicsystem:client:updateShops', -1, mech_shops)
        TriggerClientEvent('mechanicsystem:client:deleteShopMarkers', -1, data.id)
        Core.Notification(src, {
            title = '',
...
AddEventHandler('mechanicsystem:server:recruitmentRespond', function(data, args, bool, playerSrc)
    ...
    if bool == true then 
        ...
        Core.Player.SetPlayerJob(targetSrc, data.job.name, employee_grade.grade)
...
Core.RegisterCallback('mechanicsystem:server:checkMaterials', function(src, cb, materials)
    local src = Core.Player.GetSource(src)
    local hasMaterials = true
    for i = 1, #materials do
        local hasItem, hasAmount = Core.Player.HasItem(src, materials[i].name, materials[i].amount)
        if not hasItem then 
            if hasAmount == 0 then 
                Core.Notification(src, {
                    title = '',
                ...
                Core.Notification(src, {
                    title = '',
                    message = Lang['you_need_x_amount_item']:format(hasAmount, materials[i].label, materials[i].amount),
                    type = 'inform'
                })
            end
            hasMaterials = false
...
RegisterServerEvent('mechanicsystem:server:refreshVehicleBody')
AddEventHandler('mechanicsystem:server:refreshVehicleBody', function(netId, damageReport, plate)
    local src = Core.Player.GetSource(source)
    local vehicleEnt = Entity(NetworkGetEntityFromNetworkId(netId))
    vehicleEnt.state.RefreshVehicleBody = {
        report = damageReport,
        plate = plate,
        src = src,
    }
...
RegisterServerEvent('mechanicsystem:server:bodyRepairMessage')
AddEventHandler('mechanicsystem:server:bodyRepairMessage', function()
    local src = Core.Player.GetSource(source)
    Core.Notification(src, {
        title = '',
