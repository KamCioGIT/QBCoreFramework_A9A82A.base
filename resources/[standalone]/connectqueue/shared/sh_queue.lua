...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s] kreeg prioriteit en werd %d/%d in de wachtrij geplaatst", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        ...
        Queue:DebugPrint(string_format("%s[%s] is %d/%d in de wachtrij geplaatst", tmp.name, ids[1], _pos, queueCount))
    end
...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function remove()
        ...
        Queue:DebugPrint("Speler kan niet worden toegevoegd aan de verbindingslijst")
    end
    ...
    if type(id) == "table" then
        for _id, power in pairs(id) do
            if _id and type(_id) == "string" and power and type(power) == "number" then
                ...
                Queue:DebugPrint("Fout bij het toevoegen van een prioriteits-ID, ongeldige gegevens doorgegeven")
                return false
            end
        end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    Queue:CanJoin(src, function(reason)
        ...
        if reason then
            -- prevent joining
            allow = false
            done(reason and tostring(reason) or "Je bent geblokkeerd om deel te nemen")
            Queue:RemoveFromQueue(ids)
            Queue:RemoveFromConnecting(ids)
            Queue:DebugPrint(string_format("%s[%s] is geblokkeerd om deel te nemen; Reden: %s", name, ids[1], reason))
            CancelEvent()
            return
        end
    ...
    if Queue:IsInQueue(ids) then
        ...
        Queue:DebugPrint(string_format("%s[%s] is na het annuleren weer in de wachtrij gekomen", name, ids[1]))
    else
        Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    if Queue:NotFull(true) and _Queue.JoinDelay <= GetGameTimer() then
        ...
        Queue:DebugPrint(name .. "[" .. ids[1] .. "] wordt in de server geladen")
    ...
    while true do
        ...
        if not data or not data.deferrals or not data.source or not pos then
            remove("[Wachtrij] Verwijderd uit wachtrij, wachtrijgegevens ongeldig :(")
            Queue:DebugPrint(tostring(name .. "[" .. ids[1] .. "] werd uit de wachtrij verwijderd omdat ze ongeldige gegevens hadden"))
            return
        end
        ...
        if data.timeout >= Config.QueueTimeOut and os_time() - connectTime > 5 then
            remove("[Wachtrij] Verwijderd vanwege time-out")
            Queue:DebugPrint(name .. "[" .. ids[1] .. "] is uit de wachtrij verwijderd omdat er een time-out was opgetreden")
            return
        end
    ...
    while true do
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and not endPoint) or data.timeout >= Config.ConnectTimeOut) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                remove(data)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] is verwijderd uit de verbindingswachtrij omdat er een time-out is opgetreden")
            else
                i = i + 1
            end
        end
